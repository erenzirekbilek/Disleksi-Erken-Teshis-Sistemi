...
class TurkishSoundex:
    ...
    TURKISH_CHAR_MAP = {
        ...
        'b': 'B', 'c': 'C', 'ç': 'C', 'd': 'D', 'f': 'F', 'g': 'G', 'ğ': 'G',
        'h': 'H', 'j': 'J', 'k': 'K', 'l': 'L', 'm': 'M', 'n': 'N', 'p': 'P',
        'r': 'R', 's': 'S', 'ş': 'S', 't': 'T', 'v': 'V', 'w': 'W', 'x': 'X', 'y': 'Y', 'z': 'Z'
    }
    ...
    def encode(self, word: str) -> str:
        ...
        prev_code = self.CODE_MAP.get(self.TURKISH_CHAR_MAP.get(first_letter, first_letter.upper()), '')
...
class VisualSimilarityChecker:
    ...
    def weighted_levenshtein(self, s1: str, s2: str) -> float:
        ...
        for i, c1 in enumerate(s1):
            ...
            for j, c2 in enumerate(s2):
                ...
                substitutions = previous_row[j] + (0.0 if c1.lower() == c2.lower() else weight)
                current_row.append(min(insertions, deletions, substitutions))
    ...
    def get_most_similar_word(self, word: str, word_list: List[str]) -> Optional[Tuple[str, float]]:
...
class TurkishSyllableSplitter:
    ...
    VOWEL_GROUPS = ['ai', 'ei', 'ıi', 'oi', 'öi', 'ui', 'üe', 'ao', 'eo', 'ıo', 'uo', 'üe']
    ...
    def detect_split_errors(self, text: str) -> Dict:
        ...
        common_splits = {
            'kel': 'em', 'ok': 'ul', 'hi': 'ka', 'ye',
            'ba': 'şka', 'da': 'ha', 'gi': 'bi', 'ni'
        }
...
class TurkishWordValidator:
    ...
    def _load_basic_dictionary(self) -> set:
        ...
        words = {
            've', 'ile', 'için', 'ama', 'çünkü', 'ya', 'da', 'de', 'ki', 'ne', 'bu', 'o', 'biz',
            'sen', 'onlar', 'bizim', 'sizin', 'onların', 'bu', 'şu', 'o', 'bir', 'iki', 'üç',
            'dört', 'beş', 'altı', 'yedi', 'sekiz', 'dokuz', 'on', 'yirmi', 'otuz', 'kırk',
            'elli', 'altmış', 'yetmiş', 'seksen', 'doksan', 'yüz', 'bin', 'milyon', 'milyar',
            'ev', 'okul', 'kitap', 'kalem', 'silgi', 'defter', 'masa', 'sandalye', 'kapı',
            'pencere', 'duvar', 'çatı', 'baca', 'komşu', 'aile', 'anne', 'baba', 'çocuk',
            'kardeş', 'dede', 'nine', 'amca', 'teyze', 'dayı', 'hala', 'bebek', 'dost',
            'araba', 'bisiklet', 'tren', 'uçak', 'gemi', 'otobüs', 'tren', 'metro', 'taksi',
            'su', 'ekmek', 'et', 'tavuk', 'balık', 'peynir', 'yumurta', 'süt', 'çay', 'kahve',
            'mektup', 'telefon', 'bilgisayar', 'televizyon', 'radyo', 'gazete', 'dergi',
            'kitap', 'roman', 'şiir', 'hikaye', 'masal', 'fabl', 'oyun', 'film', 'müzik',
            'resim', 'spor', 'futbol', 'basketbol', 'voleybol', 'tenis', 'yüzme', 'koşu',
            'yürüyüş', 'dağ', 'deniz', 'göl', 'nehir', 'orman', 'çayır', 'tarla', 'bağ',
            'bahçe', 'çiçek', 'ağaç', ' yaprak', 'dal', 'gövde', 'kök', 'meyve', 'sebze',
            'elma', 'armut', 'muz', 'portakal', 'limon', 'üzüm', 'karpuz', 'kavun', 'çilek',
            'domates', 'biber', 'salatalık', 'soğan', 'sarımsak', 'patates', 'havuç',
            'güneş', 'ay', 'yıldız', 'bulut', 'rüzgar', 'yağmur', 'kar', 'dolu', 'gökkuşağı',
            'kış', 'yaz', 'ilkbahar', 'sonbahar', 'pazartesi', 'salı', 'çarşamba', 'perşembe',
            'cuma', 'cumartesi', 'pazar', 'bugün', 'yarın', 'dün', 'bugün', 'şimdi', 'sonra',
            'önce', 'her zaman', 'hiç', 'bazen', 'sık sık', 'çok', 'az', 'biraz', 'tamamen',
            'hiçbir', 'bazı', 'her', 'herkes', 'hiçkimse', 'kimse', 'başkası', 'başka',
            'ben', 'sen', 'o', 'biz', 'siz', 'onlar', 'benim', 'senin', 'onun', 'bizim',
            'sizin', 'onların', 'bu', 'şu', 'o', 'şu', 'bu', 'mavi', 'kırmızı', 'yeşil',
            'sarı', 'siyah', 'beyaz', 'turuncu', 'mor', 'pembemsi', 'kahverengi', 'gri',
            'büyük', 'küçük', 'orta', 'geniş', 'dar', 'uzun', 'kısa', 'kalın', 'ince',
            'ağır', 'hafif', 'sıcak', 'soğuk', 'ılık', 'yumuşak', 'sert', 'taze', 'bayat',
            'tatlı', 'ekşi', 'tuzlu', 'acı', 'iyi', 'kötü', 'güzel', 'çirkin', 'doğru',
            'yanlış', 'yeni', 'eski', 'genç', 'yaşlı', 'zengin', 'fakir', 'aç', 'tok',
            'hast', 'sağlıklı', 'yorgun', 'dinç', 'mutlu', 'üzgün', 'kızgın', 'sakin',
            'heyecanlı', 'korkmuş', 'şaşkın', 'meraklı', 'çalışkan', 'tembel', 'savurgan',
            'cimri', 'dürüst', 'yalancı', 'akıllı', 'aptal', 'zeki', 'salak', 'hakkında',
            'önünde', 'arkasında', 'yanında', 'altında', 'üstünde', 'arasında', 'içinde',
            'dışında', 'üzerinde', 'sayesinde', 'yüzünden', 'karşı', 'göre', 'göre',
            'kadar', 'gibi', 'ile', 'veya', 'fakat', 'lakin', 'halbuki', 'oysaki', 'demek',
            'ki', 'de', 'da', 'mi', 'mı', 'mu', 'mü', 'hani', 'yani', 'işte', 'evet',
            'hayır', 'peki', 'tamam', 'olur', 'sağol', 'teşekkürler', 'rica ederim',
            'afiyet olsun', 'hoşça kal', 'görüşürüz', 'merhaba', 'selam', 'alo', 'tam',
            'yarım', 'çeyrek', 'bütün', 'tamamen', 'kısmen', 'hiç', 'neredeyse', 'sanki',
            'belki', 'muhtemelen', 'muhakkak', 'kesinlikle', 'elbette', 'tabii', 'tabi',
            'evvela', 'sonra', 'önce', 'beraber', 'birlikte', 'ayrı', 'farklı', 'aynı',
            'benzer', 'diğer', 'öbür', 'sıradan', 'özel', 'genel', 'karma', 'saf', 'temiz',
            'kirli', 'boş', 'dolu', 'dolu', 'yok', 'var', 'olmak', 'yapmak', 'vermek',
            'almak', 'gelmek', 'gitmek', 'düşünmek', 'bilmek', 'istemek', 'görmek',
            'duymak', 'söylemek', 'yazmak', 'okumak', 'öğrenmek', 'öğretmek', 'anlamak',
            'hatırlamak', 'unutmak', 'başlamak', 'bitirmek', 'devam etmek', 'durmak',
            'kalkmak', 'oturmak', 'yatmamak', 'uynamak', 'yürümek', 'koşmak', 'uçmak',
            'yüzmek', 'çıkmak', 'inmek', 'geçmek', 'dönmek', 'gelmek', 'varmak', 'yetmek',
            'acıkmak', 'susamak', 'uyumak', 'uyanmak', 'gülmek', 'ağlamak', 'korkmak',
            'sevmek', 'beğenmek', 'saymak', 'hesaplamak', 'çözmek', 'bulmak', 'aratmak',
            'bulmak', 'vermek', 'almak', 'taşımak', 'kaldırmak', 'indirmek', 'açmak',
            'kapatmak', 'çevirmek', 'döndürmek', 'itmek', 'çekmek', 'atma', 'tutma',
            'bırakmak', 'yakalamak', 'vurmak', 'dövmek', 'kesmek', 'biçmek', 'delmek',
            'çizmek', 'boyamak', 'silmek', 'yıkamak', 'temizlemek', 'ütülemek', 'ütü',
            'pişirmek', 'hazırlamak', 'yemek', 'içmek', ' içecek', ' kokmak', 'tutmak',
            'taşımak', 'koymak', 'almak', 'bırakmak', 'çıkarmak', 'takmak', 'giymek',
            'soyunmak', 'giyinmek', 'beklemek', 'hızla', 'yavaş', 'hızlı', 'yavaşça',
            'dikkatli', 'özenle', 'sabırla', 'heyecanla', 'korkuyla', 'sevinçle', 'üzüntüyle',
            'öfkeyle', 'sakinle', 'net', 'belirgin', 'görünür', 'gizli', 'açık', 'kapalı',
            'kilitli', 'serbest', 'bağlı', 'bağımsız', 'mümkün', 'imkansız', 'gerekli',
            'gereksiz', 'önemli', ' önemsiz', 'faydalı', 'zararlı', 'güvenli', 'tehlikeli'
        }
        return words
...
class DyslexiaTextMetrics:
            ...
            for i, char in enumerate(word):
                if char in self.visual_checker.VISUAL_PAIRS:
                    for j, ref_char in enumerate(word):
                        if i != j and ref_char in self.visual_checker.VISUAL_PAIRS.get(char, []):
    ...
    def _analyze_structure(self, text: str) -> Dict:
        ...
        for word in words:
            ...
            if len(clean) > 10:
                common_suffixes = ['lerin', 'ların', 'nın', 'nin', 'da', 'de', 'den', 'dan']
            ...
            if len(clean) < 4 and len(clean) > 0:
                if clean in ['da', 'de', 'mi', 'mı', 'mu', 'mü', 'ye', 'ni', 'na', 'ne']:
    ...
    def _calculate_risk_score(self, metrics: Dict) -> float:
        ...
        fusion_rate = structure.get('fusion_candidates', 0) / max(structure.get('word_count', 1), 1)
        score += fusion_rate * 1.5
        
        split_rate = structure.get('split_candidates', 0) / max(structure.get('word_count', 1), 1)
        score += split_rate * 1.0
        
        unusual_rate = structure.get('unusual_length_count', 0) / max(structure.get('word_count', 1), 1)
